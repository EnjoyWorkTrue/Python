...
##################
    def draw(self,window,month):           
        ...
        self.get_ExistDay(self.year,month)
        self.draw_ExistDay(window)
...
#####################
    def update(self,data):
        self.allevent = []
        for i in data:
            self.allevent.append(i.split('-'))
    def get_ExistDay(self,year,month):
        year = str(year)
        self.drawExistDay = []
        if month > 10:
            month = str(month)
            ...
            month = '0' + str(month)
        self.monthmonth = month
        for i in self.allevent:
            if i[0] == year and i[1] == month:
                self.drawExistDay.append(int(i[2]))
    def update1(self,date,window):
        if date not in self.drawExistDay:
            a = Line(Point(self.date[date-1].anchor.getX()-5,self.date[date-1].anchor.getY()+10),Point(self.date[date-1].anchor.getX()+5,self.date[date-1].anchor.getY()+10))
            self.a.append(a)
            a.draw(window)
            if date<10:
                date = '0'+str(date)
            self.allevent.append([str(self.year),self.monthmonth,str(date)])

    def draw_ExistDay(self,window):
        self.a = []
        for i in self.drawExistDay:
            self.a.append(Line(Point(self.date[i-1].anchor.getX()-5,self.date[i-1].anchor.getY()+10),Point(self.date[i-1].anchor.getX()+5,self.date[i-1].anchor.getY()+10)))
        [i.draw(window) for i in self.a]
    def undraw_ExistDay(self):
        [i.undraw() for i in self.a]
